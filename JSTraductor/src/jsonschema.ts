import { DSL, Model, Property, JSONSchema } from "./dsl.js";

function toJSONSchemaType(prop: Property): JSONSchema {
  if (prop.$ref) {
  ...
  switch (prop.type) {
    ...
    case "array": {
      if (!prop.items) throw new Error("Array sin 'items'");
      return {
        type: "array",
...
function toJSONSchemaProp(prop: Property): JSONSchema {
  const base = toJSONSchemaType(prop);
  if (prop.description) (base as any).description = prop.description;
  if (prop.minLength !== undefined) (base as any).minLength = prop.minLength;
  if (prop.maxLength !== undefined) (base as any).maxLength = prop.maxLength;
  if (prop.minimum   !== undefined) (base as any).minimum   = prop.minimum;
  if (prop.maximum   !== undefined) (base as any).maximum   = prop.maximum;
  if (prop.pattern) (base as any).pattern = prop.pattern;
  return base;
}

function modelToSchema(name: string, m: Model): JSONSchema {
  ...
  if (m.dependencies) {
    ...
    if (allOf.length) (schema as any).allOf = allOf;
  }

  return schema;
}

export function dslToJSONSchema(dsl: DSL): JSONSchema {
  ...
  return {
    ...
    properties: Object.fromEntries(
      Object.keys(dsl.models).map(k => [k, { $ref: `#/definitions/${k}` }])
    ),
